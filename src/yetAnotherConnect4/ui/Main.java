...
public class Main {
	...
	private static void makeMenus() {
		...
		mainMenu.addOption(Color.GREEN, true, "Train network", ()->{
			...
			for(int i = 0; i<j; i++) {
				System.out.println(ansi().fgBrightCyan().a("Epoch: ").fgBright(Color.WHITE).a((i+1)).reset());
		...
		mainMenu.addOption(Color.RED, true, "Wipe Game History", ()->{gameHistory.wipe();});
	...
	public static final BoardEventListener recordKeeper = new BoardEventListener() {
		...
		public void onWinner(Board b, Player p, Win w) {
			...
			if(observe.equals(ObserveMode.END_ONLY)) {
				ConsoleUI.drawBoardFromScratch(gm);
				sleep(250);
			}
			if(agentA==agentB && agentB!=null) {
				agentA.train(gameHistory);
			}else {
				if(agentA!=null)
					agentA.train(gameHistory);
				if(agentB!=null)
					agentB.train(gameHistory);
			}
		}
